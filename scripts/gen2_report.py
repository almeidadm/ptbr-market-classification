"""Agrega os runs Gen 2 em `artifacts/runs/` e compara com o campeão Gen 1.

Lê `metadata.json` + `predictions.csv` de cada `*__gen2__*` e gera:

- `artifacts/reports/gen2_summary.csv`: uma linha por run Gen 2 com
  métricas, latência, VRAM e config.
- `artifacts/reports/gen2_vs_gen1_champion.csv`: McNemar pareado de cada
  modelo Gen 2 contra o campeão Gen 1 (default:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if gen2.empty:
        ...
        return 1
    gen2 = gen2.sort_values(["model", "variant"])
